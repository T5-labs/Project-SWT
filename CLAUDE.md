...
Tests live in the Project-SWT repo (gitignored), NOT the work repo. QA generates a shared `playwright.config.ts` at the tests root on first use — it uses `BASE_URL` as an env var so it works for any project.

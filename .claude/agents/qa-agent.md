...
3. Check for:
   ...
   - **Compound `.Where()` predicates** — flag instances where a single `.Where(x => A && B)` predicate could be split into chained `.Where(x => A).Where(x => B)` calls for readability. Semantics are equivalent in EF Core (both compose into the same SQL `AND`). Surface as a QA finding only — do not enforce proactively in SWE work.
   - **Vague variable names** — flag standalone variable names that don't convey what they represent (e.g., `duplicate`, `temp`, `data`, `result`, `flag`, `item`, `value`). Suggest a specific name that makes the intent clear (e.g., `duplicate` → `typeNameAlreadyExists` when checking for an existing record by name). Local boolean flags especially benefit from intention-revealing names. Surface as a QA finding only — do not enforce proactively in SWE work.
   - **Inline comments in our diff** — flag inline code comments (`//` in C#/TS/SCSS, `/* */` blocks, `<!-- -->` in HTML) that appear inside lines added or changed by the current ticket's diff, and recommend removing them per the codebase convention "default to no comments — only add one when the WHY is non-obvious." Scope is strict: only comments introduced by this ticket's changes — do NOT touch pre-existing comments. Keep: XML doc comments (`///` summary blocks on public API surfaces), EF migration auto-scaffolded comments, license headers, pragma/region markers, and SQL comments inside `migrationBuilder.Sql()` strings that provide real stored-procedure context. Borderline "WHY" comments (explaining genuinely non-obvious intent) can stay but should be flagged for human override. Surface as a QA finding only — do not enforce proactively in SWE work.
